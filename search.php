...
if($dept!='NULL'){
        $dept_sql = "WHERE department.Dept_Name = '".$dept."'";
}
else{
        $dept_sql = "WHERE ";
}
...
if($keyword!=''){////新增
        if($dept!='NULL')
                $keyword_sql = " AND course.Course_Name LIKE '%$keyword%'";
        else
                $keyword_sql = "course.Course_Name LIKE '%$keyword%'";
}//

if($class!=''){////新增                         #開課班級系列:如果上一頁有選擇開課班級的話，就加入塞選開課班級的SQL；如果沒有選，就不加入塞選開課班級的SQL
        if($dept!='NULL')
                $class_sql = "AND course.Class =".$class_num;
        else
                $class_sql ="course.Class =".$class_num;
}//
...
if(!isset($order_stat)){                #排序系列($orderby是附加在查詢課程資料的SQL尾端的SQL語法，$order_stat則是用來判斷現在是什麼排序)
        $orderby ="ORDER BY C_ALL DESC,c_name ASC";        #默認排序狀態：尚未排序時，預設為依照C_ALL(總分)DESC排序
...
$dept_sql$class_sql$keyword_sql #開課班級系列:若有選擇開課班級，SQL語法擺放於此，沒有選擇為空值
